...
skills/mcp_server.py  (MCP Server)
    ...
    ├── tool: run_analyze      # 读取队列 → AI 分析 → 写入 ocr_done.csv
    └── tool: run_write_ledger # 读取完成表 → 追加写入 ledger.xlsx
```
...
```
[阶段 1 - 独立运行]              [阶段 2 - 独立运行]
  watcher.py                       analyzer_runner.py
  监听 inbox/                       轮询 ocr_queue.csv
      │                                    │
      ▼                                    ▼
  ocr_batch.py                     analyze_batch.py
  OCR 识别                          gemini-cli 分析
      │                                    │
      ▼                                    ▼
  ocr_queue.csv ──────────────▶   ocr_done.csv
  (待分析队列)                      (已处理/失败)
                                           │
                                           ▼
                                    write_ledger.py
...
│   ├── analyze_batch.py    # Tool: 批量 AI 分析 ocr_queue.csv
│   ├── write_ledger.py     # Tool: 将 ocr_done.csv 已完成行写入账本
...
| `ocr_text` | OCR 原始文字 |
| `ocr_at` | OCR 时间戳 |
| `ai_result` | AI 返回的原始 JSON 字符串 |
| `status` | `success` / `error` |
| `error_msg` | 失败原因（成功时为空） |
| `analyzed_at` | AI 分析时间戳 |
| `written_to_ledger` | `true` / `false`，是否已写入账本 |
...
| `raw_input` | string | OCR 原始文字 |
| `image_path` | string | 原始图片绝对路径 |
| `processed_at` | datetime | 系统处理时间戳 |

**金额符号规则：**
- 支出：图片显负数→填正数，图片显正数（退款/抵扣）→填负数
- 收入/互转：填正数

**拆行规则：**
- 含手续费 → 主交易（互转）+ 手续费（支出，dest=「手续费」）
- 信用卡还款含优惠返现 → 主交易（互转）+ 返现（收入，source=对应银行）

---

## 🧠 提示词（prompts.yaml）

记账 AI 的完整提示词，含账户列表、金额符号规则、拆行规则和 JSON 输出要求。详见 [prompts.yaml](prompts.yaml)。
...
import ocr_batch, analyze_batch, write_ledger

mcp = FastMCP("bookkeeping")

@mcp.tool()
def run_ocr() -> str:
    """对 inbox/ 中所有新图片执行 OCR，结果追加到 ocr_queue.csv"""
    return ocr_batch.run()

@mcp.tool()
def run_analyze() -> str:
    """读取 ocr_queue.csv 未分析行，调用 AI 分析，写入 ocr_done.csv"""
    return analyze_batch.run()

@mcp.tool()
def run_write_ledger() -> str:
    """将 ocr_done.csv 中成功行写入 ledger.xlsx"""
...
gemini-cli 自动读取，定义可用工具与执行规则。详见 [GEMINI.md](GEMINI.md)。
...
python skills/analyze_batch.py    # 阶段2：AI 分析
python skills/write_ledger.py     # 阶段3：写入账本
```
...
| `analyze_batch.py` | `ocr_queue.csv` 未分析行 | `ocr_done.csv` 新行 | 调用 gemini-cli 分析，每行独立处理 |
| `write_ledger.py` | `ocr_done.csv` 未入账行 | `ledger.xlsx` 新行 | 解析 JSON，支持单笔/多笔拆分写入 |
